...
def parse_args():
    ...
    parser.add_argument('--explanation-dir', 
                        type=str, 
                        default='submodular_results/tradition-detector-correctly/yolo_v3-1.0-1.0-division-number-50',
...
def visualization(image, S_set, saved_json_file, vis_image, class_name="object", index=None, mode="insertion"):
    ...
    if index == None:
        if mode == "insertion":
            ours_best_index = np.argmax(curve_cls_score)
        elif mode == "deletion":
    ...
    if mode == "insertion":
        ...
        mask = (S_set_add.sum(0) - S_set_add[:ours_best_index+1].sum(0) + S_set_add[1] ).sum(-1).astype('uint8')
    elif mode == "deletion":
    ...
    # if mode == "insertion":
    #     cv2.imwrite("search_abstract_{}.png".format(saved_json_file["category"]), image_debug)
...
def main(args):
    ...
    for json_file_name in tqdm(json_file_names):
        ...
        # cv2.imwrite("abstract_{}.png".format(saved_json_file["category"]), vis_saliency_map)
        
        target_box = saved_json_file["target_box"]
        target_label = saved_json_file["target_label"]
        ...
        # target_category = "pool table"
        
        vis_saliency_map_w_box = annotate_with_grounding_dino(vis_saliency_map, np.array([target_box]), [target_category])
        
        visualization(image, S_set, saved_json_file, vis_saliency_map_w_box, target_category, index=7)
        plt.savefig(save_full_visualization_map_insertion_path, bbox_inches='tight',pad_inches=0.0)
        plt.clf()
        plt.close()
        
        visualization(image, S_set, saved_json_file, vis_saliency_map_w_box, target_category, mode="deletion")
