...
def parse_args():
    ...
    parser.add_argument('--save-dir', 
                        type=str, default='./baseline_results/florence-2-coco-detection-correct/',
...
def main(args):
    ...
    for explainer in explainers:
        ...
        for info in tqdm(select_infos[193:]):
            if os.path.exists(
            ...
            explainer.model.model.input_ids = processor(text=task_prompt, images=image, return_tensors="pt")['input_ids'][0].to(device)
            ...
            np.save(os.path.join(save_dir, info["file_name"].replace("/", "_").replace(".jpg", "_{}.npy".format(info["id"]))), saliency_map)
