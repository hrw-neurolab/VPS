...
def parse_args():
    ...
    parser.add_argument('--eval-dir', 
                        type=str, default='./baseline_results/florence-2-coco-detection-correct/HsicAttributionMethod/',
...
def main(args):
    ...
    for info in tqdm(select_infos[:]):
        ...
        for i in range(1, args.steps+1):
            ...
            with torch.no_grad():
                results = florence2(model, task_prompt, insertion_image)
                boxes = torch.tensor(results[task_prompt]['bboxes'])
                phrases = results[task_prompt]['labels']
                ...
                if boxes.shape[0] == 0:
                    ...
                    cls_score = np.array(phrases) == target_label
                    cls_score = cls_score.astype(float)
                    
                    insertion_scores = (ious * cls_score).max(dim=-1)[0]
                    
                    insertion_idx = (ious * cls_score).argmax().cpu().item()
                    ...
                    json_file["insertion_cls"].append(cls_score[insertion_idx])
            ...
            with torch.no_grad():
                results = florence2(model, task_prompt, deletion_image)
                boxes = torch.tensor(results[task_prompt]['bboxes'])
                phrases = results[task_prompt]['labels']
                ...
                if boxes.shape[0] == 0:
                    ...
                    json_file["deletion_cls"].append(cls_score[deletion_idx])
