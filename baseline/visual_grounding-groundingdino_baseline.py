...
def parse_args():
    ...
    parser.add_argument('--save-dir', 
                        type=str, default='./baseline_results/grounding-dino-refcoco-correct/',
...
class GroundingDino_VisualGrounding_Adaptation_Xplique(torch.nn.Module):
    def __init__(self, 
                 detection_model,
                 device = "cuda"):
        super().__init__()
    ...
    def forward_results(self, images):
        ...
        logits = prediction_logits.max(-1)[0]   # [batch,np]
    ...
    def forward(self, images):
        ...
        scores = (ious * logits).max(dim=-1, keepdim=True)[0]
...
def main(args):
    ...
    detection_model = GroundingDino_VisualGrounding_Adaptation_Xplique(model)
