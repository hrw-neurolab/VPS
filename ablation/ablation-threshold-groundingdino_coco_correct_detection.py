...
from interpretation.ablation_submodular_detection import DetectionSubModularExplanation
...
def parse_args():
    ...
    parser.add_argument('--save-dir', 
                        type=str, default='./submodular_results/ablation-threshold-0.1-grounding-dino-correctly/',
...
class GroundingDino_Adaptation(torch.nn.Module):
    ...
    def forward(self, images, h, w):
        ...
        # # Get the maximum value along the last dimension (dim=2)
        # max_values, _ = torch.max(prediction_logits, dim=2, keepdim=True)

        # # Create a mask where the maximum value is greater than the threshold
        # mask = max_values > self.threshold

        # # Apply the mask to filter logits
...
def main(args):
    ...
    detection_model = GroundingDino_Adaptation(model, args.threshold)
