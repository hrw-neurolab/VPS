...
from utils import LVIS_RARE_TEXT_PROMPT_SPLIT_1, LVIS_RARE_TEXT_PROMPT_SPLIT_2, LVIS_RARE_TEXT_PROMPT_SPLIT_3, LVIS_RARE_TEXT_PROMPT_SPLIT_4, LVIS_RARE_TEXT_PROMPT_SPLIT_5, lvis_classes_split_1, lvis_classes_split_2, lvis_classes_split_3, lvis_classes_split_4, lvis_classes_split_5, lvis_classes_grounding_idx_split1, lvis_classes_grounding_idx_split2, lvis_classes_grounding_idx_split3, lvis_classes_grounding_idx_split4, lvis_classes_grounding_idx_split5, mkdir
...
def parse_args():
    ...
    parser.add_argument('--save-dir', 
                        type=str, default='./submodular_results/grounding-dino-lvis-v1-misclassification/',
...
def main(args):
    ...
    select_infos = val_file["case2"][args.begin : end]
    for info in tqdm(select_infos):
        ...
        if info["category"] in lvis_classes_split_1:
            lvis_classes_grounding_idx = lvis_classes_grounding_idx_split1
            smdl.detection_model.caption = caption1
        elif info["category"] in lvis_classes_split_2:
            lvis_classes_grounding_idx = lvis_classes_grounding_idx_split2
            smdl.detection_model.caption = caption2
        elif info["category"] in lvis_classes_split_3:
            lvis_classes_grounding_idx = lvis_classes_grounding_idx_split3
            smdl.detection_model.caption = caption3
        elif info["category"] in lvis_classes_split_4:
            lvis_classes_grounding_idx = lvis_classes_grounding_idx_split4
            smdl.detection_model.caption = caption4
        elif info["category"] in lvis_classes_split_5:
            lvis_classes_grounding_idx = lvis_classes_grounding_idx_split5
            smdl.detection_model.caption = caption5
        
        target_class = lvis_classes_grounding_idx[info["category"]]
        ...
        # Save npy file
        np.save(
            os.path.join(save_npy_root_path, info["file_name"].replace("/", "_").replace(".jpg", "_{}.npy".format(info["id"]))),
            np.array(S_set)
        )
