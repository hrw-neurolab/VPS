...
def parse_args():
    ...
    parser.add_argument('--eval-list',
                        type=str,
                        default='datasets/coco_florence-2_correct_detection.json',
                        help='Datasets.')
    ...
    parser.add_argument('--save-dir', 
                        type=str, default='./submodular_results/florence-2-coco-detection-correctly/',
...
class Florence_2_Adaptation(torch.nn.Module):
    ...
    def forward(self, images, h, w):
        ...
        if self.text_similarity:
            target_text = clip.tokenize([self.target_class]).to(self.device) # [1, 77]
            target_text_features = self.text_model.encode_text(target_text)
        ...
        for generated_text in generated_texts:
            ...
            if self.text_similarity:
                parsed_text = clip.tokenize(parsed_answer[self.task_prompt]['labels']).to(self.device) # [3, 77]
                parsed_text_features = self.text_model.encode_text(parsed_text)
                parsed_text_features /= parsed_text_features.norm(dim=-1, keepdim=True)
                
                logits = (target_text_features @ parsed_text_features.T)[0]
                logits = (logits-0.7) / 0.3
            else:
                logits = torch.from_numpy(np.array(parsed_answer[self.task_prompt]['labels'])==self.target_class).float().to(self.device)
        ...
        prediction_logits = torch.stack(prediction_logits) # prediction_logits.shape = (batch, nq, 256)
...
def main(args):
    ...
    task_prompt = "<OD>"
    detection_model = Florence_2_Adaptation(model, 
    ...
    for info in tqdm(select_infos):
        if os.path.exists(
            os.path.join(save_json_root_path, info["file_name"].replace("/", "_").replace(".jpg", "_{}.json".format(info["id"])))
        ):
            continue
        ...
        smdl.detection_model.input_ids = processor(text=task_prompt, images=image, return_tensors="pt")['input_ids'][0].to(device)
